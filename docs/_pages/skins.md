...
{% for post in valid_skin -%} 
 ...
 <img class="card-img-top" src="{{- post.og_image | replace: ".png", ".webp" | absolute_url  -}}" alt="{{ post.skin }}"></a>
     <div class="card-body text-center border-top">
      {%- if post.skin_author -%}
      <p class="card-text text-right"><span class="font-weight-bold text-capitalize lead">{{ post.skin | replace: "-", " " | replace: "_current", " " }}</span><br><span class="small font-weight-light font-italic text-secondary"> by {{ post.skin_author}}</span></p>
      {%- endif -%}
    </div>
