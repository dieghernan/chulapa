...
### Navbar
Configure the navbar and footer of your site. This theme supports a two-level navigation structure, and features **three different navbar styles**: `modern`, as a floating button with animation, a **classic sticky-top navbar** or `dual` that would display as `modern` on small and medium devices and a classic navbar on bigger devices. 
